...
removeAt index list
  ...
  | otherwise = (Just val, result)
  where
    result = take (index - 1) list ++ drop (index + 1) list
    val = head (take 1 (drop (index - 1) list))
...
--  in this function.
evenLists :: [[a]] -> [[a]]
evenLists = filter isEvenLength
  where
    isEvenLength :: [a] -> Bool
    isEvenLength l = even $ length l
...
dropSpaces [] = []
dropSpaces str = filter isAlphaNum str
