...
-}
strSum :: String -> Int
strSum str = sum $ map toInt $ words str
  where
    toInt :: String -> Int
...
lowerAndGreater n = count 0 0
  where
    count :: Int -> Int -> [Int] -> String
    count lower greater xs
      | null xs = show n ++ " is greater than " ++ show greater ++ " elements and lower than " ++ show lower ++ " elements"
      | n > head xs = count lower (greater + 1) $ tail xs
      | n < head xs = count (lower + 1) greater $ tail xs
      | otherwise = count lower greater $ tail xs
